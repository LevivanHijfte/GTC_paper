...
states = lapply(tumors, function(x) x@meta.data |> 
         rownames_to_column("cellid") |> 
         select(state, cellid))

allstates = bind_rows(states)
...
glioma.integrated <- FindClusters(object = glioma.integrated, resolution = 1.5) # Play around with the resolution

#tmp20 = FindAllMarkers(glioma.integrated)

saveRDS(glioma.integrated, file = "dir_integrated_tumors.RDS")
